...
internal sealed class MaintenanceService : IMaintenanceService
{
    ...
    public MaintenanceService(IFleetUOW uow, ILogger<MaintenanceService> logger)
    {
        _uow = uow;
        _logger = logger;
    }
    ...
    public async Task<MaintenanceRecordDto> CreateAsync(CreateMaintenanceRecordDto dto, Guid companyId, string createdBy)
    {
        ...
        if (dto.StartDate.HasValue && dto.EndDate.HasValue && !dto.IsCompleted)
        {
    ...
    private async Task RemoveMaintenanceAvailabilityBlockAsync(MaintenanceRecord record)
    {
        ...
        foreach (var avail in allAvailabilities)
        {
            ...
            {
                var tracked = await _uow.AircraftAvailabilityRepository.GetByIdTrackingAsync(avail.Id);
                if (tracked != null)
                {
