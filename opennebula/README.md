...
| VMs are RUNNING with correct `ONEGATE_ENDPOINT`, but `TOKENTXT` stays `null` indefinitely, `KUBECONFIG_B64` / `K8S_JOIN_COMMAND` never appear, `virsh console one-<id>` returns `cannot find character device <null>`. | The old `context/cloud-init.cp.yaml` was a `#cloud-config` YAML file, but OpenNebula's `CONTEXT.START_SCRIPT_BASE64` is a **shell-script field** — one-context decodes it and pipes it to `bash`, NOT to cloud-init. Every YAML line became a "command not found" error, so kubeadm init never ran and no `onegate vm update` was ever called (hence `TOKENTXT` stayed lazily un-generated). | Replaced YAML files with proper bash scripts: [`context/bootstrap-cp.sh`](context/bootstrap-cp.sh) and [`context/bootstrap-wk.sh`](context/bootstrap-wk.sh). Both use `set -euxo pipefail` and write a trace log to `/var/log/aircraft-<role>-bootstrap.log` for post-mortem. Debugging is via VNC (look up port via `onevm show <id> \| grep PORT`); a `RAW` serial-console block was attempted but OpenNebula 7's `domain.rng` schema rejected the inline `<serial>/<console>` XML with `Invalid RAW section`. |
| `oneflow show` reports `FAILED_DEPLOYING`, log line: `Error allocating a new virtual machine template. User X does not own a network with name: aircraft-vnet. Set NETWORK_UNAME or NETWORK_UID of owner in NIC.` | OneFlow instantiates VMs as the service *owner*, not the template creator. If `aircraft-vnet` is owned by `oneadmin` but the service ends up owned by another user (or even just gets queued via the API under a different uid), the NIC name-only lookup fails because users only see their own networks by default. | Added `NETWORK_UNAME = "oneadmin"` to the `NIC = [...]` block of both [`templates/cp.tpl`](templates/cp.tpl) and [`templates/wk.tpl`](templates/wk.tpl). If your vnet is owned by a different user, change that value (check with `onevnet show aircraft-vnet \| grep ^USER`). |
| `BOOTSTRAP_STEP=4-kubeadm-init`, `BOOTSTRAP_RC=1`, `BOOTSTRAP_FAIL_LINE=169`; kubelet log full of `dial tcp: lookup aircraft.example.com on 172.16.100.1:53: no such host`. | `bootstrap-cp.sh` was writing `controlPlaneEndpoint: "${K8S_EDGE_HOST}:6443"` into the kubeadm config (default `aircraft.example.com`). On a minione lab there is no DNS for that name, so `kubelet` can't reach the API server it was just told to register against, the control-plane pods CrashLoopBackOff, and `kubeadm init` eventually times out non-zero. | Changed `controlPlaneEndpoint` to the cp's auto-detected primary IP. `K8S_EDGE_HOST` is still kept in `certSANs` so the operator can wire a real DNS later without re-issuing the API server certificate. Also added a `kubeadm reset -f` guard at the top of step 4 so re-running on a half-bootstrapped node clears the previous attempt's etcd dir, manifests, and listener ports rather than failing preflight with `Port-6443 is in use`. |
| Bootstrap fails at step 5 with `CustomResourceDefinition "installations.operator.tigera.io" is invalid: metadata.annotations: Too long: must have at most 262144 bytes`, then `resource mapping not found for name: "default" ... no matches for kind "Installation"`. | Two stacked issues: (a) the tigera-operator manifest CRDs are large; client-side `kubectl apply` writes the entire manifest as a `last-applied-configuration` annotation and blows past Kubernetes' 256 KiB annotation limit. (b) Even when the CRDs do install, the bootstrap immediately POSTed an `Installation` CR before the CRD reached the `Established` condition, hitting a "no matches for kind" race. | Step 5 now uses `kubectl apply --server-side --force-conflicts` (no annotation written) and waits explicitly with `kubectl wait --for=condition=Established crd/installations.operator.tigera.io --timeout=120s` between the operator install and the `Installation` CR. |
| `kubectl` intermittently returns `net/http: TLS handshake timeout` or `connect: connection refused`; `kubectl get pods -A` shows control-plane static pods with `RESTARTS: 9–13` and rising; etcd container logs show repeated "raft member restarting" loops. | cp-1 is undersized. Hosting etcd + apiserver + controller-manager + scheduler + containerd + kube-proxy + tigera-operator + ingress-nginx + metrics-server in 2 GiB / 2 vCPU works *briefly* but the etcd liveness probe times out under load, etcd gets killed, apiserver follows, kubelet then thrashes restarting them. Once the cluster is in this state it cannot recover without external help — every successive restart compounds the lag. | [`opennebula/templates/cp.tpl`](opennebula/templates/cp.tpl:39) MEMORY bumped from `2048` → `4096`. (2 vCPU is fine.) For an even leaner footprint on a memory-constrained lab, omit ingress-nginx and metrics-server from `bootstrap-cp.sh` and install them later as part of the application overlay. |
| Workers report `BOOTSTRAP_STEP=2-poll-join`, `BOOTSTRAP_RC=1`, `BOOTSTRAP_FAIL_LINE=115`. `kubectl get nodes` shows only cp-1 long after the join command was published. Worker VMs are still `RUNNING` in `onevm list` but their bootstrap script is dead. | The original [`bootstrap-wk.sh`](opennebula/context/bootstrap-wk.sh) capped the join-command poll loop at 15 minutes. If cp-1's bootstrap took longer than that (e.g. because we hit some of the other issues in this table on the first attempt), the workers exited with `FATAL: control-plane never published K8S_JOIN_COMMAND`. The VMs themselves keep RUNNING — only the bootstrap process is dead — so OneFlow doesn't auto-replace them. | Made the worker poll loop **unbounded** (it was bounded to `for i in $(seq 1 90); do … sleep 10; done`, now `while true; do … sleep 10; done`). A heartbeat publishes `BOOTSTRAP_STEP=2-poll-join (N polls so far, ~Ns)` every 60s so the operator can still see the worker is alive. Recovery for stuck workers: `onevm terminate <wk-vmid>` for each — OneFlow will replace them, and the replacements will see the join command on their first poll. |
