...
public class TenantResolutionMiddleware
{
        ...
        ICompanyService companyService)
    {
        ...
        if (context.User?.Identity?.IsAuthenticated == true)
        {
            ...
            {
                await AutoSelectFirstCompanyAsync(context, tenantService);
            }
            else
            {
                ...
                if (Guid.TryParse(cookieValue, out var cookieCompanyId))
                {
                    ...
                    if (companyActive)
                    {
                        ...
                        {
                            userStillMember = await tenantService.IsUserInCompanyAsync(cookieCompanyId, cookieUserId);
                        }
                    }
        ...
        if (!string.IsNullOrEmpty(slug))
        {
            ...
            // Resolve slug to company ID using application service
            var tenantId = await tenantService.ResolveTenantIdFromSlugAsync(slug);
        ...
        ITenantService tenantService)
    {
        try
        {
            ...
            // Get user's companies via application service
            var userCompanies = await tenantService.GetUserCompaniesAsync(userId);
            ...
            context.Response.Cookies.Append("SelectedCompanyId", firstCompany.CompanyId.ToString(), new CookieOptions
            {
            ...
            context.Items["TenantId"] = firstCompany.CompanyId;
            context.Request.Headers["X-Tenant-Id"] = firstCompany.CompanyId.ToString();

            _logger.LogDebug("Auto-selected company '{CompanyId}' for user '{UserId}'",
                firstCompany.CompanyId, userId);
        }
        catch (Exception ex)
        {
